...
class Ai():
    ...
    def highlight(self, nodes):
        for n in nodes:
            rect = pygame.Rect(n[0], n[1], Settings.food_size, Settings.food_size)
            self.screen.fill((73, 139, 167), rect)
        time.sleep(0.01)

    def get_shortest_path(self, target_x, target_y):
        visited = set()
        queue = []
        head_x, head_y = self.snake.get_head_position()
    ...
    def get_path(self, target_x, target_y): # a* algorithm
        ...
        head_x, head_y = self.snake.get_head_position()
        queue.put((0, 0, head_x, head_y, []))
        while not queue.empty():
            _, _, x, y, path = queue.get()
            # self.highlight(visited)
            if x == target_x and y == target_y:
            ...
            if (x, y) not in visited:
                ...
                for d in directions:
                    next_x, next_y = self.snake.get_move_result(x, y, d)
                    p = self.get_priority(next_x, next_y)
                    queue.put((p + 1, time.time(), next_x, next_y, path + [d]))
        return []
